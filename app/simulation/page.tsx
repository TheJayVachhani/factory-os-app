...
export default function SimulationPage() {
  ...
  const handleTriggerEvent = async (eventId: string) => {
    // 1. Apply event effect to state
    const res = await fetch(`/api/sim/event/${eventId}`, { method: "POST" });
    const data = await res.json() as SimState;
    setState(data);

    // 2. Clear previous agent output
    setToolCalls([]);
    setAgentResponse("");
    setRecommendations([]);
    setIsAnalyzing(true);

    // 3. Close any existing SSE
    sseRef.current?.close();

    // 4. Open SSE stream for agent analysis
    const sse = new EventSource(`/api/sim/analyze?eventId=${eventId}`);
    sseRef.current = sse;
